...
async function sendMpesaOrderReceivedEmail(orderData) {
  ...
  try {
    ...
    ` : `
      <p style="color:#888;font-size:14px;">Your M-PESA payment of <strong style="color:#2ecc71;">KES ${(total || 0).toLocaleString()}</strong> has been received!</p>
...
<div style="background:#111118;border-radius:24px;overflow:hidden;border:1px solid #1e1e2c;">
  ...
  <div style="padding:20px 28px;">
    <h2 style="color:#fff;font-size:18px;">Hello ${escapeHtml(customerName || 'Customer')},</h2>
    ${messageHtml}
  </div>
  <div style="padding:0 28px;">
    <table style="width:100%;background:#16161f;border-radius:16px;overflow:hidden;">
      ...
      ${itemsHtml}
      <tr><td style="padding:12px 16px;color:#777;">Subtotal</td><td style="padding:12px 16px;text-align:right;color:#ccc;">KES ${(subtotal || 0).toLocaleString()}</td></tr>
      ...
      <tr style="background:#0a1a0a;"><td style="padding:16px;color:#fff;font-weight:800;">${totalLabel}</td><td style="padding:16px;text-align:right;color:${totalColor};font-size:20px;font-weight:800;">KES ${(total || 0).toLocaleString()}</td></tr>
    </table>
  </div>
...
async function sendOTPEmail(email, otp) {
  if (!BREVO_API_KEY) {
    console.warn('BREVO_API_KEY not configured - OTP email not sent');
    return false;
  }
